...
public class SupplyAdapter extends BaseQuickAdapter<SupplyListEntity, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, SupplyListEntity item) {
        ...
        .setText(R.id.tv_time,String.format("%s%s   %s",item.getFreight_price(),StrUtil.formatMoneyUnit(item.getUnit()),StrUtil.formatSettlementTime(item.getLog_settlement_time())));
        helper.setText(R.id.logic_name,"物流："+item.getLogistical_name());
    }
}
